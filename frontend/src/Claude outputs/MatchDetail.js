 ...
 * 분야: 프론트엔드 / 페이지
 * 기능: 경기 상세 + 개인 스탯 입력/조회
 * ====================================
 *
 * 경기 카드에서 "상세/스탯 보기"를 누르면 이 페이지로 와.
 *
 * URL이 /matches/1 이면 id=1인 경기의 상세 정보를 보여줘.
 * useParams()를 써서 URL에서 id를 가져오는데,
 * Android에서 Intent의 putExtra/getExtra로 데이터를 전달했던 것과 비슷해.
 *
 * 여기서 선수별 골, 어시스트를 입력하고 조회할 수 있어.
 */
...
function MatchDetail() {
    const { id } = useParams();     // URL에서 경기 ID 가져오기
    ...
    async function handleAddStat(e) {
        ...
        try {
            ...
            setNewStat({ memberId: '', goals: 0, assists: 0 });
            // 스탯 목록만 다시 불러오기
            ...
            <div className="stats-section">
                ...
                {stats.length > 0 && (
                    <div className="stats-summary">
                        총 {totalGoals}골 / {totalAssists}어시스트
                    </div>
                )}
